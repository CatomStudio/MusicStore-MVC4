...
namespace Dapper.Contrib.Extensions1
{
    ...
    public static class SqlMapperExtensions
    {
        ...
        // 属性容器
        private static readonly ConcurrentDictionary<RuntimeTypeHandle, IEnumerable<PropertyInfo>> AllProperties = new ConcurrentDictionary<RuntimeTypeHandle, IEnumerable<PropertyInfo>>();
        // 可计算字段容器
        ...
        private static readonly Dictionary<string, ISqlAdapter> AdapterDictionary = new Dictionary<string, ISqlAdapter>() { { "mysqlconnection", new MySqlServerAdapter() } };
        // db格式容器
        private static readonly Dictionary<string, SqlEscape> EscapeDictionary = new Dictionary<string, SqlEscape>() { { "mysqlconnection", new SqlEscape() { LeftEscape = "`", RightEscape = "`" } } };
        #endregion
        ...
        private static IEnumerable<PropertyInfo> ComputedPropertiesCache(Type type)
        {
            ...
            var computedProperties = AllPropertiesCache(type).Where(p => p.GetCustomAttributes(true).Any(a => a is ComputedAttribute)).ToList();
        ...
        private static IEnumerable<PropertyInfo> KeyPropertiesCache(Type type)
        {
            ...
            // 双管齐下，避免失误。
            var allProperties = AllPropertiesCache(type);
            // 方法一
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        private static IEnumerable<PropertyInfo> AllPropertiesCache(Type type)
        {
            IEnumerable<PropertyInfo> pis;
            if (AllProperties.TryGetValue(type.TypeHandle, out pis))
            {
                return pis;
            }

            var properties = type.GetProperties().Where(IsWriteable).ToArray();
            AllProperties[type.TypeHandle] = properties;
            return properties;
        }

        private static string InsertQueriesCache(Type type, SqlEscape escape = null)
        {
            string sql;
        ...
        private static string GetUpdatePartQuery(Type type, object entity, SqlEscape escape = null)
        {
            var keyProperties = KeyPropertiesCache(type);
            if (!keyProperties.Any())
                throw new ArgumentException("Entity must have at least one [Key] property");
        ...
        private static string GetUpdatePartQuery(Type type, IDictionary<string, object> source, IDictionary<string, object> condition, SqlEscape escape = null)
        {
            var name = GetTableName(type);
        ...
        #region 3. CRUD 的映射实现机制，System.Data.IDBConnection 的扩展方法
        /// <summary>
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (type.IsInterface)
            {
                ...
                foreach (var property in AllPropertiesCache(type))
                {
